...
fov_n_voxels_xyz = torch.tensor([128, 128, 1])
...
mu_tensor_plate = geoms[geoms[:, 6] == 0, 7].unsqueeze(0).expand(n_panels, -1)
mu_tensor_detector = geoms[geoms[:, 6] != 0, 7].unsqueeze(0).expand(n_panels, -1)
...
dset[crystal_indices[rank].numpy()] = partial_matrix.cpu().numpy()
f.close()
time_end = MPI.Wtime()
print(
